var indexApp = {
				...
				if(t.valueJson['is_gz'] == 1 && t.valueJson['is_follow'] == 2) {
					...
					/*$.ajax({
						...
						success : function (data) {*/
							var num=Math.round(Math.random()*400);
							var mess="化妆品";
							var actionStatus=1;
							var status=1;
							console.log(num);
							var thisNumber=$('.g-num').find('em').text();
							if(thisNumber==0){
								status=2;
								actionStatus=3;
							}
							var data = {'status' : status, 'actionStatus' : actionStatus, 'ran' : num, 'onceran' : num, 'num' : 1, 'mess' : mess}
	...
	var t = this;
		$('.g-num').find('em').html(this.number-data['num']);
		this.number=this.number-data['num'];
	},
	...
	dialog : function (obj, data, bl) {
		...
		console.log(data['mess']);
			obj.find('d-main').children('p').text("Hello world!");
		}
